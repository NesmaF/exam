...
function showDetails(mealDetails) {
  ...
  for (let i = 1; i < 20; i++) {
    if (
      ...
      ingre += `<span>${mealDetails[`strMeasure${i}`]} ${
        mealDetails[`strIngredient${i}`]
      }</span>`;
    }
  }
...
function displayCategories(categoryMeals) {
  ...
  for (let i = 0; i < categoryMeals.length; i++) {
    let description = categoryMeals[i].strCategoryDescription
      .split(" ")
      .slice(0, 20)
      .join(" ");
...
function displayIngredients(mealsIngredients) {
  ...
  for (let i = 0; i < mealsIngredients.length; i++) {
    let descriptionIn = mealsIngredients[i].strDescription
      .split(" ")
      .slice(0, 20)
      .join(" ");
...
$("#emailInput").on("input", function () {
  ...
  if (!emailRegex.test(emailValue)) {
    $(".email-alert")
      .removeClass("d-none")
      .text("Email not valid (e.g., example@yyy.zzz)");
    $("#submitBtn").prop("disabled", true);
  } else {
